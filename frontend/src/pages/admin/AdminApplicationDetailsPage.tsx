...
import { ArrowLeft, CalendarDays, CircleUserRound, Mail, MessageCircle } from 'lucide-react';
...
export function AdminApplicationDetailsPage() {
  ...
  return <div className="admin-section admin-detail"><Link className="back-link" to="/admin/applications"><ArrowLeft size={16} /> Все заявки</Link><div className="page-heading page-heading--actions"><div><span className="eyebrow">ЗАЯВКА</span><h1>№ {app.number}</h1><p>Подана {formatDateTime(app.created_at)}</p></div><StatusBadge status={app.status} /></div>
    <div className="details-grid"><Card className="detail-card"><h2>Заявитель</h2><div className="contact-list"><span><CircleUserRound size={18} /><strong>{app.applicant.snapshot_full_name}</strong></span><span><Mail size={18} />{app.applicant.snapshot_email}</span>{app.applicant.telegram ? <span><MessageCircle size={18} />{app.applicant.telegram}</span> : null}{app.applicant.max ? <span><MessageCircle size={18} />MAX: {app.applicant.max}</span> : null}</div></Card>
    ...
    <div className="details-grid"><Card className="detail-card"><h2>Файлы</h2><FileList files={app.files} onDownload={download} downloadingId={downloadingId} />{downloadError ? <Alert>{downloadError}</Alert> : null}</Card><Card className="detail-card detail-card--wide"><h2>История</h2><div className="timeline">{app.status_history.map((entry, index) => <div className="timeline-item" key={`${entry.created_at}-${index}`}><span className="timeline-dot" /><div><strong>{STATUS_LABELS[entry.status]}</strong><span className="timeline-meta"><CalendarDays size={14} /> {formatDateTime(entry.created_at)} · {entry.changed_by ? `${entry.changed_by.full_name || 'Без имени'} (${entry.changed_by.role === 'admin' ? 'администратор' : 'пользователь'})` : 'Системное изменение'}</span>{entry.comment ? <p>{entry.comment}</p> : null}</div></div>)}</div></Card></div>

...
function validateFiles(files: File[]): string | null {
  ...
  for (const file of files) {
    ...
    if (file.size > MAX_FILE_SIZE) return `${file.name}: размер превышает 100 МБ.`;
  }
  return null;
}
...
export function NewApplicationPage() {
  ...
  return <section className="page page--wizard"><button className="back-link back-link--button" onClick={() => navigate(-1)}><ArrowLeft size={16} /> Назад</button>
    ...
    <Card className="wizard-card">
      ...
      {step === 3 ? <div className="form-stack"><h2>Файлы и желаемая дата</h2><label className="upload-zone"><FileUp size={30} /><strong>Добавьте модели</strong><span>STL, STEP, STP, 3MF или ZIP, до 100 МБ каждый · максимум 10 файлов. Вместо загрузки можно указать ссылку ниже.</span><input ref={fileInput} type="file" multiple accept=".stl,.step,.stp,.3mf,.zip" disabled={create.isPending} onChange={(event) => addFiles(event.target.files)} /><Button type="button" variant="secondary" disabled={create.isPending} onClick={() => fileInput.current?.click()}>Выбрать файлы</Button></label>

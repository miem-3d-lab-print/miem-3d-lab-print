...
import { ArrowLeft, CalendarDays, CircleUserRound, MessageSquareText, Upload } from 'lucide-react';
...
function validateFile(file: File): string | null {
  ...
  if (!ALLOWED_FILE_EXTENSIONS.includes(extension)) return 'Разрешены только STL, STEP, STP и 3MF.';
...
export function ApplicationDetailsPage() {
  ...
  const upload = useMutation({ mutationFn: (file: File) => applicationsApi.uploadFile(id, file), onSuccess: async () => { setFileError(''); if (inputRef.current) inputRef.current.value = ''; await queryClient.invalidateQueries({ queryKey: ['application', id] }); } });
  ...
  return <section className="page page--detail"><Link className="back-link" to="/app/applications"><ArrowLeft size={16} /> Мои заявки</Link>
    <div className="page-heading page-heading--actions"><div><span className="eyebrow">ЗАЯВКА</span><h1>№ {app.number}</h1><p>Подана {formatDateTime(app.created_at)}</p></div><StatusBadge status={app.status} /></div>
    ...
    <Card className="detail-card"><h2>Файлы</h2><FileList files={app.files} onDownload={download} downloadingId={downloadingId} />
      {app.status === 'new' ? <div className="upload-inline"><input ref={inputRef} type="file" accept=".stl,.step,.stp,.3mf" onChange={(event) => handleFile(event.target.files?.[0])} /><Button variant="secondary" loading={upload.isPending} disabled={app.files.length >= MAX_FILES_PER_APPLICATION} onClick={() => inputRef.current?.click()}><Upload size={17} /> Добавить файл</Button><small>{app.files.length >= MAX_FILES_PER_APPLICATION ? `Достигнут лимит ${MAX_FILES_PER_APPLICATION} файлов.` : `До 20 МБ. Файлов: ${app.files.length}/${MAX_FILES_PER_APPLICATION}. После перехода в рассмотрение файлы блокируются.`}</small></div> : null}

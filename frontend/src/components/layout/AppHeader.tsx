    ...
    <header className="site-header">
      ...
      <div className={`header-actions ${menuOpen ? 'header-actions--open' : ''}`}>
        <nav className="main-nav">
          ...
          {isAuthenticated ? <NavLink to="/app/applications">Мои заявки</NavLink> : null}
          {profile?.role === 'admin' ? <NavLink to="/admin/applications">Админ-панель</NavLink> : null}
        </nav>
        {isAuthenticated ? (
          <div className="header-user-actions">
            <Button size="sm" onClick={() => navigate('/app/applications/new')}><Plus size={17} /> Оформить заявку</Button>
            <Link className="avatar-button" to="/profile" title="Мой профиль">
          ...
          <Button variant="dark" size="sm" onClick={() => navigate('/auth/login')}><UserRound size={17} /> Войти</Button>
        )}
      </div>
    </header>
  );
